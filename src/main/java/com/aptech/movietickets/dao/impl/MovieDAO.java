...
public class MovieDAO extends BaseDAO<MovieModel> implements IMovieDAO {
    ...
    public int save(MovieModel movieModel) {
        StringBuilder sql = new StringBuilder("INSERT INTO `movie`(`genres`, `release`, `duration`, `cast`, `director`, `title`,`thumbnail`) ");
        sql.append("VALUES (?,?,?,?,?,?,?)");
    ...
    public void update(MovieModel updateMovie) {
        String sql = "UPDATE `movie` SET `genres`= ?,`release`= ?,`duration`= ?,`cast`= ?,`director`= ?,`title`= ?,`thumbnail`= ? WHERE id = ?";
        update(sql,
